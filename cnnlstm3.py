...
def wordCleaner(x):
	...
	while i < len(x):
		if not x[i].isdigit():
			...
			r = i
			while r < len(x) and x[r].isdigit(): r += 1
			ret += "<"+str(r-1-i+1)+">"
			i = r
	return ret
...
for instance in trainSentences:
	...
	trainingLabels_6dig.append(labels_6dig.index(labs[:]))
...
for instance in tuneSentences:
	...
	tuningLabels_6dig.append(labels_6dig.index(labs[:]))
...
for instance in testSentences:
	...
	testingLabels_6dig.append(labels_6dig.index(labs[:]))
...
trainingLabels_6dig = to_categorical(trainingLabels_6dig,nb_classes=len(labels_6dig))
...
testingLabels_6dig = to_categorical(testingLabels_6dig,nb_classes=len(labels_6dig))
...
tuningLabels_6dig = to_categorical(tuningLabels_6dig,nb_classes=len(labels_6dig))
...
labelsFile_6dig = codecs.open('labels.txt','w',encoding='latin-1')
labelsFile_6dig.write("\n".join(labels_6dig))
labelsFile_6dig.close()
...
x = Merge(mode = 'concat')([y,out1,out2])
x = Dense(hiddenSize)(x)
x = BatchNormalization()(x)
x = Activation('relu')(x)
x = Dense(len(labels_6dig))(x)
...
for e in range(epochs):
	...
	model.fit(trainingInstances,[trainingLabels_2dig,trainingLabels_4dig,trainingLabels_6dig],batch_size=batchSize,nb_epoch=1,verbose=verb,class_weight=['auto','auto','auto'])
	...
	scores = model.evaluate(tuningInstances,[tuningLabels_2dig,tuningLabels_4dig,tuningLabels_6dig])
	...
	for i in range(len(tuningInstances)):
		...
		y_true = 0
		for j in range(len(tuningLabels_6dig[i])):
			if tuningLabels_6dig[i][j] > 0.5:
				y_true = j
				break
